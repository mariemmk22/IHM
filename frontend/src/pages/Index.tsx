...
const getCategoryIcon = (categoryName: string): LucideIcon => {
  ...
  if (normalized.includes("electr")) return Zap;
  if (normalized.includes("plomb") || normalized.includes("canalis") || normalized.includes("sanitaire")) return Droplets;
  if (normalized.includes("peint") || normalized.includes("deco")) return Paintbrush;
  if (normalized.includes("bricol") || normalized.includes("renovat") || normalized.includes("maçon")) return Hammer;
  if (normalized.includes("menage") || normalized.includes("nettoy") || normalized.includes("proprete")) return Sparkles;
  if (normalized.includes("coiff") || normalized.includes("beaute")) return Scissors;
  if (normalized.includes("transport") || normalized.includes("demenag") || normalized.includes("livraison")) return Truck;
  if (normalized.includes("jardin") || normalized.includes("paysag")) return Leaf;
  if (normalized.includes("maison") || normalized.includes("domicile")) return Home;
  if (normalized.includes("repar") || normalized.includes("maintenance") || normalized.includes("installation")) return Wrench;
        ...
        <div className="container mx-auto px-4 py-16">
          ...
          >
            {(servicesLoading ? Array.from({ length: 6 }) : filteredServices.slice(0, 6)).map((service, index) => (
              <motion.div key={(service as { id?: string })?.id ?? index} variants={fadeUp}>
                {servicesLoading ? (
                  <div className="h-80 rounded-xl bg-secondary animate-pulse" />
                ) : (
                  <ServiceCard
                    id={(service as { id: string }).id}
                    title={(service as { nomService: string }).nomService}
                    provider={(() => {
                      const typed = service as {
                        prestataire?: { id?: string; nom?: string; prenom?: string };
                      };

                      if (!typed.prestataire) return "Prestataire";

                      const fullName = `${typed.prestataire.nom ?? ""} ${typed.prestataire.prenom ?? ""}`.trim();
                      return fullName || `Prestataire #${typed.prestataire.id ?? "?"}`;
                    })()}
                    category={(service as { sousCategorie?: { nom?: string } }).sousCategorie?.nom ?? "Service"}
                    region={(service as { region: string }).region}
                    price={`${(service as { prix: number | string }).prix} DA`}
                    rating={0}
                    reviewCount={0}
                    image={getServiceImage({
                      nomService: (service as { nomService: string }).nomService,
                      category: (service as { sousCategorie?: { nom?: string } }).sousCategorie?.nom,
