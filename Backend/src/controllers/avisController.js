	...
	try {
		const avis = await Avis.findAll({
			include: [
					...
					include: [
						{
							model: Service,
							as: "service",
							attributes: ["idService", "nomService"],
						},
						{
							model: Client,
							as: "client",
				...
				{ model: Note, as: "note" },
				{ model: Commentaire, as: "commentaire" },
			],
			order: [["dateAvis", "DESC"]],
		});
		...
		const normalized = avis.map((item) => ({
			...
			comment: item.commentaire?.description ?? "",
			client: item.rendezVous?.client
				? { nom: item.rendezVous.client.nom, prenom: item.rendezVous.client.prenom }
				: null,
			prestataire: item.rendezVous?.prestataire
				? {
						nom: item.rendezVous.prestataire.client?.nom ?? "",
						prenom: item.rendezVous.prestataire.client?.prenom ?? "",
						specialite: item.rendezVous.prestataire.specialite,
					}
				: null,
			service: item.rendezVous?.service
...
exports.deleteAvis = async (req, res) => {
	try {
		const { id } = req.params;
		const avis = await Avis.findByPk(Number(id));
